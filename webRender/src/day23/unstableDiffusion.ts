...
export function performRound(
  ...
  [...mapOfProposedElfMoves.values()].forEach((elfMoveArray) => {
    if (elfMoveArray.length == 1) {
      mapOfElfLocations.delete(
        JSON.stringify(elfMoveArray[0]!.currentLocation)
      );
