export class RingBuffer<T> {
  ...
  /**
   * Shifts an item, by 'offset' positions, wrapping if required.
   */
  public shiftItem(item: T, offset: number) {
    const itemIndex = this.buffer.indexOf(item);
    if (itemIndex === -1) {
      return;
    }

    const arr = this.toArray();
    const removedItems = arr.splice(itemIndex, 1);

    let destinationIndex = itemIndex + offset;
    ...
    arr.splice(destinationIndex, 0, ...removedItems);
    this.fromArray(arr);
  }
